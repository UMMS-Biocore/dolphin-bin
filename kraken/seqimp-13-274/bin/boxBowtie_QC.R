...
for (sampleChoice in names(sumValues)){
   ...
   readDistRepeats <- matrix(data=0, nrow = length(classRecords[["Repeats"]]), ncol = ncol(readDistTable), dimnames = list("row" = classRecords[["Repeats"]],"col" = colnames(readDistTable)))
   ...
   readDistRepeats <- readDistTable[rownames(readDistRepeats),,drop=FALSE]
   readDistGenes <- readDistTable[rownames(readDistGenes),,drop=FALSE]
   if (cycle == 0){
   ...
   bowtieSummary(t(readDistRepeats[repeatOrder,]), repeatCols, repeatTitle, yaxisLimit)

   geneTitle <- paste("Reads mapping to genic features as a proportion\nof the reads that map to an annotation class for", sampleChoice)
   bowtieSummary(t(readDistGenes[geneOrder,]), geneCols, geneTitle, yaxisLimit)
...
for (chrmSel in chrmFiles){
   ...
   barplot(as.matrix(chrPlot) , beside=TRUE, col=plot_colours,ylim =c(min(chrPoints),max(chrPoints)), ylab="Percentage of processed reads" , xlab="Toplevel Ensembl chromosomes" ,main=barTitle, axes = FALSE, cex.axis = 0.75, cex.main=0.75, las = 2)
