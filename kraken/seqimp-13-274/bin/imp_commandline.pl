...
die "Require a 'configuration' file (--user-configuration=<FILE> OR --default_cofiguration=<FILE>)\n" if ((length($userConfig)==0) && (length($defaultConfig)==0));
die "Only require either --user-configuration=<FILE> OR --default_cofiguration=<FILE>"
                  if ((length($userConfig)!=0) && (length($defaultConfig)!=0));
